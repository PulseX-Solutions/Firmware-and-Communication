...
void loop() {
  ...
  if (now - lastMsg > 4000) {
    lastMsg = now;
    float EOG = random(0, 10001) / 100.0;
    char msg[10];
    sprintf(msg, "%.2f", EOG); // Convert float to string
    client.publish("CurrentEOGValue", msg);


    // client.publish("CurrentEOGValue", EOG); //topic name (to which this ESP32 publishes its data). 88 is the dummy value.
    Serial.println("Published EOG Value "+ String(EOG));
